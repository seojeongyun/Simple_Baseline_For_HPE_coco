...
class PoseResNet(nn.Module):
    ...
    def forward(self, x):
        # x.shape(input x) is torch.Size([4, 3, 256, 256])
        ...
        x = self.layer2(x) # x.shape is torch.Size([4, 512, 32, 32])
        x = self.layer3(x) # x.shape is torch.Size([4, 1024, 16, 16])
        x = self.layer4(x) # x.shape is torch.Size([4, 2048, 8, 8])

        x = self.deconv_layers(x) # x.shape is torch.Size([4, 256, 64, 64])
        x = self.final_layer(x) # x.shape is torch.Size([4, 17, 64, 64])
